...
router.post('/api/projects/share', async ({ request, user }) => {
    try {
        ...
        if (!id || !action) return jsonError(400, 'Project ID and action are required');
        if (action !== 'share' && action !== 'unshare') return jsonError(400, 'Action must be "share" or "unshare"');
        ...
        const updatedProject = {
            ...
            sharedAt: action === 'share' ? new Date().toISOString() : null,
            sharedBy: action === 'share' ? userId : null,
            updatedAt: new Date().toISOString(),
        };
